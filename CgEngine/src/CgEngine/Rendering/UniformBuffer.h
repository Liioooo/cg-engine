...
namespace CgEngine {
    ...
    private:
        ...
        void init(const std::string& blockName, uint32_t binding, uint32_t shaderForInitRendererId) {
            ...
            CG_LOGGING_INFO("Uniform Buffer: {0}, OpenGL Size: {1}, C++ Size: {2}", blockName, bufferSize, sizeof(D))
            CG_ASSERT(sizeof(D) == bufferSize, "UniformBuffer" + blockName + ", Buffer size mismatch!")
