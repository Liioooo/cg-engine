...
void main() {
    ...
    if (k_length <= u_cutoffHigh && k_length >= u_cutoffLow) {
        ...
        h0 = imageLoad(u_gaussianNoise, texelCoord).xy * sqrt(2.0 * JONSWAP(k_length) * deltaK * deltaK * abs(dOmegadk) / k_length);
    }
...
float frequencyDerivative(float k) {
    float th = tanh(min(k * u_depth, 20.0));
    float ch = cosh(k * u_depth);
    return u_g * (u_depth * k / ch / ch + th) / omega(k) / 2.0;
}

float omega(float k) {
    return sqrt(u_g * k * tanh(min(k * u_depth, 18.0)));
}
...
float JONSWAP(float k) {
    ...
    float S = u_alpha * u_g * u_g * omega_recip * omega_recip * omega_recip * omega_recip * omega_recip * exp(-1.25 * pow(u_omega_p * omega_recip, 4.0)) * pow(u_gamma, r);
    return S;
}
