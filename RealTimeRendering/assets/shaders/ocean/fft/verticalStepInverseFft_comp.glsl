...
void main() {
    ...
    vec4 data = imageLoad(precomputedData, ivec2(_step, id.y));
    uvec2 inputsIndices = uvec2(data.ba);
    if (pingPong) {
        vec2 value = imageLoad(buffer0, ivec2(id.x, inputsIndices.x)).xy
        ...
        vec2 value = imageLoad(buffer1, ivec2(id.x, inputsIndices.x)).xy
